...
const navItems = [
  ...
  },
  {
    ...
    icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 9h16"/><path d="M4 15h16"/><path d="M10 3L8 21"/><path d="M16 3l-2 18"/></svg>,
  },
];

const adminNavItems = [
  {
    href: '/users', label: 'Gestione Utenti',
    icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 15c-4.42 0-8 1.79-8 4v2h16v-2c0-2.21-3.58-4-8-4z"/><circle cx="12" cy="7" r="4"/><path d="M20 8v6M23 11h-6"/></svg>,
    adminOnly: true,
  },
];
...
export function Sidebar() {
  ...
  const allNav = [...navItems, ...(user?.role === 'admin' ? adminNavItems : [])];
    ...
    <div className="p-4 mx-3 mb-3">
      {user ? (
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white text-sm font-bold shadow-sm ${user.role === 'admin' ? 'bg-gradient-to-br from-amber-400 to-amber-600' : 'bg-gradient-to-br from-brand-400 to-brand-600'}`}>
            {(user.name || user.username || '?').charAt(0).toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-surface-800 truncate">{user.name} {user.surname}</p>
            <p className="text-[11px] text-surface-400 capitalize">{user.role === 'admin' ? '👑 Super Admin' : '📋 Agente'}</p>
          </div>
          <button onClick={handleLogout} className="p-2 hover:bg-red-50 rounded-lg transition-colors text-surface-400 hover:text-red-500" title="Logout">
        ...
        <div className="h-10 bg-surface-100 rounded-xl animate-pulse" />
      )}
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-30 bg-white border-b border-surface-200 flex items-center justify-between px-4 py-3">
        ...
        <button onClick={() => setMobileOpen(!mobileOpen)} className="p-2 rounded-lg hover:bg-surface-50 transition-colors" aria-label="Toggle menu">
          {mobileOpen
      ...
      {mobileOpen && <div className="lg:hidden fixed inset-0 bg-black/40 z-40 animate-fade-in" onClick={() => setMobileOpen(false)} />}

      <aside className={`fixed top-0 bottom-0 w-[260px] bg-white border-r border-surface-200 flex flex-col z-50 transition-transform duration-300 ease-out ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:left-0`}>
        {/* Desktop logo */}
        <div className="hidden lg:block p-6 pb-2">
          <Link href="/dashboard" className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center shadow-md shadow-brand-200">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
            </div>
            <div>
        ...
        <nav className="flex-1 px-4 pt-4 lg:pt-6 overflow-y-auto">{navContent}</nav>
        {userSection}
      </aside>
    </>
  );
